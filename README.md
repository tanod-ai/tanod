...
Returns one approval request by id. Polling integrations use this endpoint to wait for `approved`, `rejected`, or `expired`. Approved records include the signed `approval_token` needed to retry `/v1/executions`.
