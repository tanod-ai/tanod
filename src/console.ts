...
<head>
  ...
  <style>
    ...
    input, select, button, textarea { border-radius: .5rem; border: 1px solid #334155; background: #020617; color: #e2e8f0; padding: .6rem .75rem; }
    ...
    .empty { border: 1px dashed #475569; border-radius: .75rem; padding: 1rem; margin: 1rem 0; color: #cbd5e1; background: #0b1220; }
    ...
    .badge.pending { background: #854d0e; }
    .badge.approved { background: #166534; }
    .badge.rejected { background: #7f1d1d; }
    .badge.expired { background: #475569; }
...
<body>
  <main>
    ...
    <div class="toolbar">
      ...
      <button onclick="saveApiKeyAndRefresh()">Refresh</button>
    </div>
    <div id="status"></div>
...
apiKeyEl.value = localStorage.getItem('tanod.apiKey') || '';
approverEl.value = localStorage.getItem('tanod.approver') || approverEl.value;
...
function saveApiKeyAndRefresh() {
  localStorage.setItem('tanod.apiKey', apiKeyEl.value.trim());
  localStorage.setItem('tanod.approver', approverEl.value.trim());
  loadApprovals();
}

async function loadApprovals() {
  const selectedStatus = statusFilterEl.value;
  statusEl.textContent = 'Loading approval requests...';
  approvalsEl.innerHTML = '';
  try {
    const query = selectedStatus ? '?status=' + encodeURIComponent(selectedStatus) : '';
    const res = await fetch('/v1/approval-requests' + query, { headers: headers() });
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || res.statusText);
    const records = data.approval_requests || [];
    const label = selectedStatus || 'all';
    statusEl.innerHTML = '<span class="ok">' + records.length + ' ' + escapeHtml(label) + ' approval request(s)</span>';
    approvalsEl.innerHTML = records.map(renderApproval).join('') || renderEmptyState(selectedStatus);
  } catch (err) {
    statusEl.innerHTML = '<span class="error">' + escapeHtml(String(err.message || err)) + '</span>';
    approvalsEl.innerHTML = '<div class="empty"><b>Could not load approval requests.</b><p>If Tanod was started with <code>TANOD_API_KEYS</code>, enter the matching API key above and click Refresh.</p></div>';
  }
}

function renderEmptyState(selectedStatus) {
  const statusText = selectedStatus || 'any';
  return '<div class="empty">'
    + '<b>No ' + escapeHtml(statusText) + ' approval requests found.</b>'
    + '<p>Approval requests only appear here after creating one via <code>tanod request-approval &lt;request.json&gt; --by &lt;user&gt;</code> or <code>POST /v1/approval-requests</code>.</p>'
    + '<p class="muted">A plain <code>tanod decide</code> call only evaluates policy; it does not create a persistent approval request.</p>'
    + '</div>';
}
...
function renderApproval(record) {
  const actions = record.status === 'pending'
    ? '<div class="actions">'
      ...
      + '</div>'
    : '<p class="muted">No actions available for ' + escapeHtml(record.status) + ' requests.</p>';
  return '<article class="card">'
    + '<h2>' + escapeHtml(record.request.tool.name) + ' <span class="badge ' + escapeHtml(record.status) + '">' + escapeHtml(record.status) + '</span> <span class="badge">' + escapeHtml(record.decision.risk_level) + '</span></h2>'
    + '<div class="meta">'
...
async function approve(id) {
  ...
  localStorage.setItem('tanod.approver', approved_by);
...
async function rejectApproval(id) {
  ...
  localStorage.setItem('tanod.approver', rejected_by);
